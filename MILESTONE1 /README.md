...
3. **Multi-Route Identity Recovery (Forgot Password Page)**
...
4. **Role-Based Post-Auth Dashboards**
...
1. Open the primary project notebook inside Google Colab.
2. Run the initial configuration cells to install the mandatory external dependency packages (`pyjwt`, `bcrypt`, `streamlit-option-menu`, `plotly`).
3. Execute the `%%writefile app.py` cell block to compile the frontend interface logic, database initializations, and layout definitions into a local script.
4. Run the background terminal activation block (`streamlit run app.py &`) alongside your network tunnel configurations (such as `ngrok` or alternative secure tunneling tools) to generate a public proxy domain url.
5. Click on the generated public link to enter and test the portal environment.
...
- **Login Viewport:** `[Link or reference to your login screenshot]`
- **Signup Interface:** `[Link or reference to your signup screenshot]`
- **Forgot Password (Security Question Route):** `[Link or reference to your SQ recovery screenshot]`
- **Forgot Password (SMTP Mail OTP Verification Engine):** `[Link or reference to your OTP layout screenshot]`
- **Received OTP Notification Email:** `[Link or reference to your email snapshot]`
- **Standard User Analytics Portal:** `[Link or reference to your dashboard layout]`
- **Administrative Privileged Console Ledger:** `[Link or reference to your admin view screenshot]`
